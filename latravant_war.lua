...
function user_setup()
	...
	state.AutoBerserk = M(false, "Auto Berserk")    
    
    war_sj = player.sub_job == 'WAR' or false
    state.drain = M(false)
	state.WeaponMode = M{['description']='Weapon Mode', 'lycurgos', 'router', 'bravura'}
    
    -- Additional local binds
    send_command('bind ^= gs c cycle treasuremode')
    send_command('bind != gs c toggle CapacityMode')
	send_command('bind !h gs c toggle AutoHasso')
	send_command('bind ^h gs c toggle AutoBerserk')
    send_command('bind !` input /ja "Seigan" <me>')
	send_command('bind ^w equip main lycurgos')
...
function init_gear_sets()
    ...
    sets.MadrigalBonus = {
     ...
     sets.precast.JA['Berserk'] = { body="Pummeler's Lorica +1", feet="Agoge Calligae +1", back=Cichols.TP}
     sets.precast.JA['Warcry'] = { head="Agoge Mask"}
     sets.precast.JA['Mighty Strikes'] = { hands="Agoge Mufflers +1"}
     sets.precast.JA['Retaliation'] = { hands="Pummeler's Mufflers +1", feet="Boii Calligae +2"}
     ...
     sets.precast.WS = {
		...
		back={ name="Cichol's Mantle", augments={'STR+20','Accuracy+20 Attack+20','Attack+5','Weapon skill damage +10%','System: 1 ID: 640 Val: 4',}},
     }
    ...
    sets.precast.WS['Savage Blade'] = set_combine(sets.precast.WS['Impulse Drive'], {
     ...
     sets.idle.Field = set_combine(sets.idle.Town, {
		...
		head={ name="Valorous Mask", augments={'Weapon skill damage +3%','Attack+19','Accuracy+14 Attack+14',}},
		body="Hjarrandi Breast.",
		hands={ name="Odyssean Gauntlets", augments={'Crit. hit damage +3%','Accuracy+27','Damage taken-4%','Accuracy+15 Attack+15','Mag. Acc.+17 "Mag.Atk.Bns."+17',}},
		legs="Sulev. Cuisses +2",
		feet="Hermes' Sandals",
     ...
     sets.defense.PDT = {
		...
		neck="Homeric Gorget",
		waist="Nierenschutz",
		left_ear={ name="Moonshade Earring", augments={'Accuracy+4','Latent effect: "Regain"+1',}},
		right_ear="Colossus's Earring",
		left_ring="Moonbeam Ring",
    ...
    sets.lycurgos = { main="lycurgos", sub="dilettante's grip +1"}
	sets.bravura = { main="bravura", sub="dilettante's grip +1"}
	sets.router = { main="router", sub="dilettante's grip +1"}
...
function check_for_berserk()
    if not (buffactive['Berserk']) then
        send_command('input /ja "Berserk" <me>')
        coroutine.schedule(check_for_berserk, 15)
    end
end
...
function job_buff_change(buff, gain)
	...
	if buff:contains("Berserk") and gain == false then
		add_to_chat(167, "*** BERSERK DOWN ***")
	end

	if  
		buff:contains("Berserk")
		and gain == false 
		and player.hp > 0
		and not areas.Cities:contains(world.area)
		and state.AutoBerserk.current == 'on'
	then	
		send_command('input /ja "Berserk" <me>')
		coroutine.schedule(check_for_berserk, 15)
	end 
...
function job_state_change(stateField, newValue, oldValue)
	...
	elseif stateField == 'Weapon Mode' then
        if newValue == 'lycurgos' then
            equip(sets.lycurgos)
        elseif newValue == 'bravura' then
            equip(sets.bravura)
		elseif newValue == 'router' then
			equip(sets.router)
        end
    end
end
